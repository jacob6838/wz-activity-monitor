...
class RecordingConfiguration extends StatelessWidget {
        ...
        body: Padding(
          ...
          child: Obx(() => ListView(children: [
                    ...
                    IconButton(
                      onPressed: () async {
                        ...
                        ProjectWithId? project =
                            await Get.to(() => const SelectProjects());
                        if (project != null) {
                ...
                Row(
                  children: [
                    SizedBox(
                      width: screenWidthPercentage(context, percentage: 0.7),
                      child: _inputField("Segment ID", segmentIdController,
                          isNumeric: true),
                    ),
                    Expanded(child: Container()),
                    IconButton(
          ...
          verticalSpaceMedium,*/
                ...
                _inputField("Recording Date", recordingDateController,
                    isDate: true, isRequired: true, context: context),
                verticalSpaceMedium,
                ...
                _inputField("Mobility Speed (MPH)", mobilitySpeedController,
                    isDouble: true),
                verticalSpaceMedium,
                _dropdownField("Surface Type", surfaceTypes, context,
                    isSurfaceType: true),
                verticalSpaceMedium,
                _inputField("Number of Lanes", numberOfLanesController,
                    isDouble: true, isRequired: true),
                verticalSpaceMedium,
                Container(
                    height: 3,
  ...
  Widget _dropdownField(
      ...
      {int? index, bool isAreaWorkType = false, bool isSurfaceType = false}) {
    ...
    return ElevatedButton(
      onPressed: () {
        ...
        RecordingController recordingController =
            Get.put(RecordingController(), tag: 'thisone'); //TODO: change tag
        recordingController.projectId = projectIdController.text != ""
            ? int.parse(projectIdController.text)
            : null; //optional
        recordingController.segmentId = segmentIdController.text != ""
            ? int.parse(segmentIdController.text)
            : null; //optional
        recordingController.areaId = areaIdController.text != ""
        ...
        recordingController.startDate =
            _parseDate(startDateController.text); //optional
        recordingController.endDate =
            _parseDate(endDateController.text); //optional
  ...
  bool _fieldsValid() {
    ...
    if (recordingDateController.text != "" &&
        _parseDate(recordingDateController.text) == null) {
      NotificationController()
          .queueNotification('Invalid Report', 'Invalid date format');
      return false;
    }
    if (areaType == "") {
      NotificationController().queueNotification(
